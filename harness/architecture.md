...
Git owns source identity. Native `goal-host-checks` and the unique composition
Story own acceptance evidence.
...
`goal-host-checks` is the deterministic acceptance boundary. It must cover:
