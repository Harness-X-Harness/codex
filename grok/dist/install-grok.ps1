$ErrorActionPreference = "Stop"
$ArchiveRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$BinDir = if ($env:GROK_BIN_DIR) { $env:GROK_BIN_DIR } else { Join-Path $env:LOCALAPPDATA "Grok\bin" }
$GrokHome = if ($env:GROK_HOME) { $env:GROK_HOME } else { Join-Path $env:USERPROFILE ".grok" }
$ConfigPath = Join-Path $GrokHome "config.toml"

if (Test-Path $ConfigPath) {
    throw "Refusing to overwrite $ConfigPath"
}

New-Item -ItemType Directory -Force -Path $BinDir, $GrokHome | Out-Null
Copy-Item (Join-Path $ArchiveRoot "bin\grok.ps1") (Join-Path $BinDir "grok.ps1")
Copy-Item (Join-Path $ArchiveRoot "bin\grok-bin.exe") (Join-Path $BinDir "grok-bin.exe")
Copy-Item (Join-Path $ArchiveRoot "bin\codex-code-mode-host.exe") (Join-Path $BinDir "codex-code-mode-host.exe")
Copy-Item (Join-Path $ArchiveRoot "config.toml.example") $ConfigPath

Write-Host "Installed Grok in $BinDir. Set GROK_API_KEY, then run grok.ps1."
