...
names the seam and the evidence. No document lists SHAs, so nothing has to be
kept in step with git.
...
Fold related commits into one semantic when they are the same decision (for
example request projection through the whitelist, or CI steps that only moved
proof). Docs fold into the semantic they document. A `fix(grok)` that stock
now owns is dropped, and its test is kept only if it still asserts a Grok
behavior. The emit / omit / reject map that must survive adoption is
[`request-whitelist.md`](./request-whitelist.md), not the old commit list.
...
3. **Replay the stack in order.** `git cherry-pick` the range from
   `git log --reverse rust-vOLD..grok/rust-vOLD`, folding related commits
   into one semantic when they are the same decision. On conflict, resolve at
   the current stock seam: read what stock now does at that seam and keep the
   Grok semantic on top of it. Never resolve by keeping a mechanism stock now
   owns, and never by merging `rust-vNEW` into the old line.
