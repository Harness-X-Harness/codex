...
```text
PR to grok/main or grok/rust-v*   = Cargo on the native PR merge ref
...
A pull request to `grok/main` or a `grok/rust-v*` candidate line runs Cargo on
GitHub's default pull-request merge ref. It does not build distribution
binaries or run real-provider Live.

A push to `grok/main` or a `grok/rust-v*` candidate line runs:
