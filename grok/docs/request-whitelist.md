...
Current emit / omit / reject map for Grok Responses egress at
`ResponsesDialect::project_request` → `grok_request::build`.

This is a maintainer design record. It is not executable acceptance input.
Runtime types live in `codex-rs/codex-api/src/grok_request.rs`. Product
semantics stay in [`architecture.md`](./architecture.md), delivery in
[`release.md`](./release.md), and stock-tag adoption in
[`carry-forward.md`](./carry-forward.md). Backend classes live in
`grok/facts` (`TestFact*`).
...
| Grok source | `grok/rust-v0.154.0` @ `1512b8a6f` (`grok-v0.154.0`) |
...
*acceptance*. A Facts run is the recorded backend class; a flip revises the
row, not a publish.
...
The emitted set is `(Codex canonical ∩ Grok accepts) ∪ Grok-native
extensions`, not a subset of the OpenAI request. Grok-native extensions enter
through stock configuration and Provider seams; they are not hard-coded at
egress.
...
5. **Unknowns are probed, not guessed.** Neither omission nor emission happens
   without an observed rejection, a recorded Fact, or a GREEN Live run.
...
maps it, and the compiler names the place.
...
constructed), **reject** (error before transport), **keep-current** (today's
egress is accepted; do not change it unless a Story needs a different wire).
...
| `parallel_tool_calls` | omit | `TestFactParallelToolCallsStoreClientMetadata` (`accepted` for `parallel_tool_calls: false`) ≠ consumed; grok-build `None` |
...
| `store: false` | omit | `TestFactParallelToolCallsStoreClientMetadata` (`accepted` for `store: false`) ≠ consumed; grok-build `None` |
...
| `client_metadata` | omit | `TestFactParallelToolCallsStoreClientMetadata` (`accepted` for `client_metadata` alone) ≠ consumed; Codex-backend telemetry with no Grok function; grok-build does not send it |
...
| `ImageGenerationCall` | `image_generation_call` | `id?`, `status`, `revised_prompt?`, `result` | image-edit Story was GREEN with `status`; `status` is accepted (`TestFactInputStatusOnHostedItems`) |
...
|-----------------|-----------|-------------------|
| `function { name, description, parameters, strict, defer_loading? }` | `function { name, description, parameters }` | omit `strict`: grok-build `strict: None`; `TestFactFunctionStrict` (`accepted`) ≠ consumed. `defer_loading` is not constructed |
...
| `web_search` domain allowlist | `filters.allowed_domains` (max 5) | stock `web_search.filters.allowed_domains` | `filters.allowed_domains` from stock when present (max 5) | — | Live (`TestGrokHostedWebSearchAllowlist`) |
| `web_search` domain blocklist | `filters.excluded_domains` (max 5, exclusive with allowlist) | stock `[tools.web_search] excluded_domains` | `filters.excluded_domains` from stock when present (max 5); omit `filters` when both lists are non-empty | — | Live (`TestGrokHostedWebSearchExcludedDomains`) |
...
| `x_search` date window | `from_date` / `to_date` (`YYYY-MM-DD`) | Grok Provider `[model_providers.grok.x_search]` | copied onto the `x_search` tool (canonical tool JSON wins when present) | — | Live (`TestGrokHostedXSearchDateWindow`) |
...
response as a backend-executed call. Codex marks any completed
...
## Recorded backend facts

Independent of the binary. `grok/facts`, first recorded 2026-09-16 against
`grok.trustedtunnel.app`. Rejected classes are
`rejected:<HTTP status>/<error>` with the backend `error` text
whitespace-collapsed and cut at 160 characters.
...
What those facts settle:

- Input items: `custom_tool_call.id` and `web_search_call.action` are
  required on replay. Dropping either is a `422`, not a silent ignore.
- Reasoning: only `content: null` next to a blob is rejected. A typed
  `reasoning_text` channel is accepted. Omission stays the conservative
  default until a Story needs the text on the wire (see keep-current).
- Function tool `strict` is omitted (`accepted` ≠ consumed).
  `parallel_tool_calls`, `store`, and `client_metadata` are omitted
  (`accepted` ≠ consumed). `text.verbosity` is already omitted.
- `web_search.filters.allowed_domains` and `filters.excluded_domains` are
  accepted and emitted from stock (max 5, exclusive). The `x_search` date
  window is accepted and emitted from Provider
  `[model_providers.grok.x_search]` (canonical tool JSON wins).

### Keep-current

Accepted either way, or accepted but not required for a user-visible Story.
Do not change egress unless a Story needs a different wire.

| Item | Current egress | Fact | Change only if |
|------|----------------|------|----------------|
| `status` on replayed `custom_tool_call`, `web_search_call`, `image_generation_call` | keep what stock records | `TestFactInputStatusOnHostedItems` (`accepted`) | a Story needs a different replay shape |
| reasoning `content` with blob | omit `content` when a blob is present | `TestFactReasoningTypedContentWithBlob` (`accepted`) | a Story needs typed `reasoning_text` on Turn N+1 |
...
- Live: `grok/live` `go test -run '^TestGrok'` on the musl binary. Named
  Stories in [`stories/`](./stories/) bind each user-visible row above.
...
A whitelist commit that lands after a GREEN run is a proof input and needs
its own GREEN run before `grok/release.py publish`. A docs-only commit does
not. Delivery is [`release.md`](./release.md).
...
its grok-build shape, choose the stock-compatible entry seam, emit through
that seam, and add a Live Story when it is user-visible. Do not hard-code it
at egress.
