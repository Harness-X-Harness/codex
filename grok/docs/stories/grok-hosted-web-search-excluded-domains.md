# Grok hosted `web_search` domain blocklist Turn and replay

## User story

As a Grok user, I complete one Turn on the shipped profile with stock
`[tools.web_search] excluded_domains` so hosted `web_search` advertises that
...
exact Grok release artifact
  ...
  -> Turn 1 /responses advertises web_search.filters.excluded_domains
  -> Turn 1 that must use hosted web_search for current news-publisher content
...
stock `filters.excluded_domains`, and its history replay, on the packaged Grok
artifact. It does not replace the default unfiltered hosted `web_search`
Story and does not complete an `allowed_domains` Story.

## Acceptance

**Given** the preconditions, **when** the acceptance runner submits one Grok
App Server Turn that cannot be answered from memory and names a news publisher
(not Wikipedia), then one follow-up Turn on the same Thread, **then**:
...
- Turn 1 (or the last `/responses` for that Turn) advertises a `tools` entry
  `web_search` with `filters.excluded_domains` containing the configured
  domain;
...
- `excluded_domains_advertised`: Turn 1's last `/responses` request does not
  advertise a `web_search` tool with `filters.excluded_domains` containing the
  configured domain.
...
This Story does not prove `web_search.filters.allowed_domains`, the default
...
- Isolated `CODEX_HOME` installs the shipped profile
  ...
  `[tools.web_search] excluded_domains`. `shell_tool` is disabled for the
  proof Turns.
