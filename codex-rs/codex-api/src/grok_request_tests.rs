...
#[test]
fn grok_whitelist_uses_explicit_dialect_not_name_or_host() {
    let mut named_grok_openai = provider("Grok");
    named_grok_openai.dialect = ApiDialect::OpenAi;
    named_grok_openai.base_url = "https://api.x.ai/v1".to_string();
    let canonical = request(vec![user_message("dialect")]);
    let identity = serde_json::to_value(&canonical).expect("stock request serializes");
    assert_eq!(
        identity["parallel_tool_calls"],
        json!(true),
        "OpenAI dialect keeps stock request keys regardless of name or host"
    );

    let mut named_openai_grok = provider("OpenAI");
    named_openai_grok.dialect = ApiDialect::Grok;
    named_openai_grok.base_url = "https://example.invalid/v1".to_string();
    let projected = build(&canonical, &named_openai_grok).expect("explicit Grok dialect");
    assert!(
        projected.get("parallel_tool_calls").is_none(),
        "Grok dialect omits OpenAI-only keys even when name and host are not Grok: {projected}"
    );
    assert!(
        projected.get("store").is_none(),
        "Grok dialect omits store even when name and host are not Grok: {projected}"
    );
}
...
fn grok_projects_replayed_history_on_request_copy_only() {
    ...
    let projected = build(&canonical, &provider("Grok")).expect("Grok history should project");
...
fn grok_omits_reasoning_content_when_replaying_encrypted_blob() {
    ...
    let projected = build(&canonical, &provider("Grok"))
...
fn grok_omits_null_encrypted_reasoning_blob() {
    ...
    let projected =
        build(&canonical, &provider("Grok")).expect("Grok should omit a null reasoning blob");
...
fn grok_strips_openai_only_history_controls_from_replay() {
    ...
    let projected =
        build(&canonical, &provider("Grok")).expect("OpenAI-only history extras should project");
...
fn grok_rejects_encrypted_collaboration_history_before_transport() {
    ...
    let error = build(&request, &provider("Grok"))
...
fn grok_projects_web_and_x_search_contract_without_touching_flat_functions() {
    ...
    let projected = build(&canonical, &provider("Grok")).expect("tool projection");
    assert_eq!(canonical, original, "canonical request must stay unchanged");
    assert_eq!(
        projected["tools"],
        json!([
...
fn grok_projects_web_search_allowed_domains_from_stock_filters() {
    ...
    let projected =
        build(&canonical, &provider("Grok")).expect("web_search allowed_domains should project");
...
fn grok_projects_web_search_excluded_domains_from_stock_filters() {
    ...
    let projected =
        build(&canonical, &provider("Grok")).expect("web_search excluded_domains should project");
...
fn grok_projects_bare_web_search_when_filters_are_missing_or_empty() {
    for (name, web_search) in [
        ...
        let projected =
            build(&canonical, &provider("Grok")).unwrap_or_else(|err| panic!("{name}: {err}"));
        assert_eq!(
            projected["tools"],
...
#[test]
fn grok_rejects_web_search_allowed_domains_over_five() {
    let mut canonical = request(vec![user_message("search")]);
    canonical.tools = Some(json_tools(json!([{
        "type": "web_search",
        "filters": {
    ...
    let error = build(&canonical, &provider("Grok"))
        .expect_err("more than five allowed_domains must fail closed");
    assert!(
        error
            .to_string()
            .contains("web_search filters.allowed_domains cannot list more than 5 domains"),
        "{error}"
    );
}

#[test]
fn grok_rejects_web_search_when_allowed_and_excluded_domains_are_both_present() {
    let mut canonical = request(vec![user_message("search")]);
    canonical.tools = Some(json_tools(json!([{
        "type": "web_search",
        "filters": {
    ...
    let error = build(&canonical, &provider("Grok"))
        .expect_err("conflicting web_search filters must fail closed");
    assert!(
...
#[test]
fn grok_rejects_non_string_web_search_domains() {
    let mut canonical = request(vec![user_message("search")]);
    canonical.tools = Some(json_tools(json!([{
        "type": "web_search",
        "filters": {
    ...
    let error = build(&canonical, &provider("Grok"))
        .expect_err("non-string web_search domains must fail closed");
    assert!(
        error
            .to_string()
            .contains("web_search filters.allowed_domains must contain only strings"),
        "{error}"
    );
}

#[test]
fn grok_rejects_web_search_excluded_domains_over_five() {
    let mut canonical = request(vec![user_message("search")]);
    canonical.tools = Some(json_tools(json!([{
        "type": "web_search",
        "filters": {
    ...
    let error = build(&canonical, &provider("Grok"))
        .expect_err("more than five excluded_domains must fail closed");
    assert!(
        error
            .to_string()
            .contains("web_search filters.excluded_domains cannot list more than 5 domains"),
        "{error}"
    );
}
...
fn grok_strips_external_web_access_from_nested_request_payloads() {
    ...
    let projected =
        build(&canonical, &provider("Grok")).expect("nested search extras should project");
...
fn stock_openai_keeps_reasoning_content_with_encrypted_blob() {
    ...
    assert_eq!(
        serde_json::to_value(&canonical).expect("stock projection"),
        expected
    );
...
fn grok_appends_bare_x_search_without_date_keys() {
    ...
    let projected = build(&canonical, &provider("Grok")).expect("bare x_search append");
    assert_eq!(
        projected["tools"],
        json!([{"type": "web_search"}, {"type": "x_search"}])
    );
...
#[test]
fn grok_rejects_invalid_x_search_dates_from_tool_json() {
    let mut canonical = request(vec![user_message("x")]);
    canonical.tools = Some(json_tools(json!([{
        "type": "x_search",
    ...
    let error =
        build(&canonical, &provider("Grok")).expect_err("invalid x_search dates must fail closed");
    assert!(
        error
            .to_string()
            .contains("x_search.from_date `not-a-date` must be a calendar YYYY-MM-DD"),
        "{error}"
    );
}

#[test]
fn grok_rejects_file_id_input_images() {
    let canonical = request(vec![ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![ContentItem::InputImage {
    ...
    let error = build(&canonical, &provider("Grok")).expect_err("file_id images must fail closed");
    assert!(error.to_string().contains("input_image.file_id"), "{error}");
}
...
fn grok_omits_unexpected_keys_on_x_search() {
    ...
    let projected =
        build(&canonical, &provider("Grok")).expect("unexpected x_search keys should be dropped");
    assert_eq!(
        projected["tools"],
        json!([{
            "type": "x_search",
