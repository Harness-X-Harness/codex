...
- `webSearch` — `{id, query, action, results, source}` for a web search request issued by the agent; `action` mirrors the Responses API web_search action payload (`search`, `open_page`, `find_in_page`) and is `null` until available. For standalone web search, `results` contains the out-of-band structured result DTOs returned by `/v1/alpha/search`; clients should ignore result types and fields they do not understand. `source` is `"x"` for an X Search backend call and `null` for ordinary web search. These nullable fields are always present on v2 item payloads.
- `imageGeneration` — `{id, status, revisedPrompt, prompt, result, transparentBackground, savedPath}` for a generated image. `revisedPrompt` contains the prompt revised by an image service and `prompt` contains the native prompt reported by a provider that does not revise it; each is `null` when unavailable. `transparentBackground` is `true` when the Images API reports a transparent background, `false` when it reports an opaque background, and `null` when the background is automatic, unavailable, or the item has not completed. `savedPath` is `null` until the image has been saved. These nullable fields are always present on v2 item payloads, including persisted and resumed items.
