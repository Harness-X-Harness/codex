...
- `thread/workflow/start` — experimental; start an independent Rhai `/workflow` run on a thread. Requires `goal_host`. Rejects if a run is already `active` or `waiting`. When Goal HOW occupies the engine, the start persists as `waiting` until the occupant is no longer `active`. The source must be valid Rhai. The host auto-resumes the VM after each yield until the program completes or the user stops it.
