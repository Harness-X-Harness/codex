...
pub trait ModelsManager: fmt::Debug + Send + Sync {
    /// Loads a catalog while preserving an unavailable authoritative source as an explicit error.
    ///
    /// Non-authoritative managers preserve their existing local catalog when an optional remote
    /// refresh fails.
...
pub struct OpenAiModelsManager {
    ...
    authoritative_refresh_lock: Mutex<()>,
...
impl OpenAiModelsManager {
    ...
    fn new_with_optional_cache(
        ...
        Self {
            ...
            authoritative_refresh_lock: Mutex::new(()),
...
impl ModelsManager for OpenAiModelsManager {
    fn load_model_catalog(
        ...
        Box::pin(OpenAiModelsManager::load_catalog(
            self,
            refresh_strategy,
            http_client_factory,
        ))
...
impl OpenAiModelsManager {
    ...
    async fn refresh_available_models(
        ...
        if self.endpoint_client.remote_catalog_is_authoritative() {
            ...
            return self
                .refresh_available_models_inner(refresh_strategy, http_client_factory)
                .await;
        }
