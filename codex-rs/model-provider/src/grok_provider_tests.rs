...
fn grok_info(name: &str) -> ModelProviderInfo {
    ModelProviderInfo {
        name: name.to_string(),
        base_url: Some("https://example.test/v1".to_string()),
...
#[test]
fn grok_provider_identity_is_wire_api_only() {
    ...
    assert!(is_grok_provider_info(&grok_info("Custom")));
}

#[test]
fn grok_provider_identity_matches_serialized_wire_selector() {
    let provider = create_model_provider(grok_info("Custom"), /*auth_manager*/ None);
    assert_eq!(provider.tool_wire_format(), ToolWireFormat::FlatFunctions);
...
async fn arbitrary_name_with_grok_responses_selects_grok_dialect() {
    ...
    assert_eq!(api_provider.name, "any-proxy");
    assert_eq!(api_provider.dialect, ApiDialect::Grok);
}

#[tokio::test]
async fn name_grok_with_responses_selects_openai_dialect() {
...
#[tokio::test]
async fn xai_host_with_responses_selects_openai_dialect() {
    let mut info = provider_info("xAI");
    info.base_url = Some("https://api.x.ai/v1".to_string());
...
#[tokio::test]
async fn arbitrary_proxy_url_with_grok_responses_selects_grok_dialect() {
    let mut info = grok_info("corp-proxy");
    info.base_url = Some("https://proxy.internal.example/v1".to_string());
    let provider = create_model_provider(info, /*auth_manager*/ None);
    let api_provider = provider
        .api_provider()
        .await
        .expect("Grok wire_api should construct an API provider");
    assert_eq!(api_provider.dialect, ApiDialect::Grok);
    assert_eq!(api_provider.base_url, "https://proxy.internal.example/v1");
}

#[tokio::test]
async fn routing_base_url_changes_do_not_mutate_dialect() {
    let mut info = grok_info("xAI");
    info.base_url = Some("https://proxy.example/v1".to_string());
    let provider = create_model_provider(info, /*auth_manager*/ None);
    let mut api_provider = provider
        ...
        .expect("Grok wire_api should construct an API provider");
    assert_eq!(api_provider.dialect, ApiDialect::Grok);
    api_provider.base_url = "https://routed.example/backend-api/codex".to_string();
    assert_eq!(api_provider.dialect, ApiDialect::Grok);
}

#[tokio::test]
async fn grok_api_provider_keeps_x_search_window_without_renaming() {
    let mut info = grok_info("xAI");
    ...
    let api_provider = provider
        ...
        .await
        .expect("Grok wire_api should construct an API provider");
    assert_eq!(api_provider.name, "xAI");
    assert_eq!(api_provider.dialect, ApiDialect::Grok);
    assert_eq!(
...
async fn grok_models_manager_uses_bundle_or_exact_config_replacement() {
    let provider = create_model_provider(grok_info("xAI"), /*auth_manager*/ None);
...
#[test]
fn name_grok_with_responses_does_not_use_grok_image_policy() {
    let provider = create_model_provider(provider_info("Grok"), /*auth_manager*/ None);
    assert_eq!(image_generation_policy(&provider), None);
}
...
fn grok_internal_tasks_use_bundled_model_not_openai_ids() {
    let grok = create_model_provider(grok_info("xAI"), /*auth_manager*/ None);
...
fn grok_ultra_resolves_to_xhigh_only_at_request_normalization() {
    ...
    assert_eq!(
        ...
        "stock request normalization projects logical Ultra to Grok xhigh"
    );
}
