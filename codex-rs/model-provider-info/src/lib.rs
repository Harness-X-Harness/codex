...
pub enum WireApi {
    ...
    ///
    /// Transport remains the stock Responses HTTP/SSE path. This variant is the
    /// only serialized selector that maps to `ApiDialect::Grok`.
...
impl WireApi {
    ...
    fn to_api_dialect(self) -> codex_api::ApiDialect {
        match self {
            Self::Responses => codex_api::ApiDialect::OpenAi,
            Self::GrokResponses => codex_api::ApiDialect::Grok,
        }
    }
}
...
other non-default provider fields are not supported"
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> CodexResult<ApiProvider> {
        ...
        Ok(ApiProvider {
            ...
            dialect: self.wire_api.to_api_dialect(),
